...
contract ExponentialLaunchpad is BaseHook {
    ...
    function beforeInitialize(address _initializer, PoolKey calldata key, uint160, bytes calldata data)
        external
        override
        returns (bytes4)
    {
    ...
    function handleExactInput(
        ...
        (specifiedAmount, unspecifiedAmount) =
            handleExactInputSwap(specifiedAmount, specified, unspecified, params.zeroForOne, key);
    ...
    function handleExactOutput(
        ...
        (unspecifiedAmount,) = handleExactOutputSwap(specifiedAmount, specified, unspecified, params.zeroForOne, key);
    ...
    function seedInitialLiquidity(PoolKey calldata key) internal {
        ...
        if (mintedTokens[id] >= tokenToMintSupply[id]) {
            ...
            if (poolToLPStartTime[id] == 0) {
                ...
                int256 delta1;
                {
                    int256 deltaBefore0 = manager.currencyDelta(address(this), key.currency0);
                    int256 deltaBefore1 = manager.currencyDelta(address(this), key.currency1);
                    ...
                    int256 deltaAfter0 = manager.currencyDelta(address(this), key.currency0);
                    int256 deltaAfter1 = manager.currencyDelta(address(this), key.currency1);
    ...
    function getCurrencies(IPoolManager.SwapParams calldata params, PoolKey calldata key, bool exactInput)
        internal
        pure
    ...
    function getSpecifiedAmount(IPoolManager.SwapParams calldata params, bool exactInput)
        internal
        pure
        returns (uint256)
    {
        ...
        if (mintedTokens[id] < tokenToMintSupply[id]) {
            ...
            if (zeroForOne) {
                int24 currTick = MINIMUM_TICK
                    + int24((MAXIMUM_TICK - MINIMUM_TICK) * int256(mintedTokens[id]) / int256(tokenToMintSupply[id]));
                ...
                int24 currTick = INVERT_MAXIMUM_TICK
                    - int24(
                        (INVERT_MAXIMUM_TICK - INVERT_MINIMUM_TICK) * int256(mintedTokens[id])
